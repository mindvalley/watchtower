...
function parseGitleaks(report, { excludePaths = [], reviewRules = [] } = {}) {
...
function parseTrivy(report, { prodDeps = [], devDeps = [] } = {}) {
  ...
  for (const res of results) {
    ...
    for (const v of vulns) {
      ...
      bump(raw, band);
      let bucket;
      if (v.PkgName && devSet.has(v.PkgName)) { bump(dev, band); bucket = 'dev'; }
      else if (v.PkgName && prodSet.has(v.PkgName)) { bump(prod, band); bucket = 'prod'; }
      else { bump(transitive, band); bucket = 'transitive'; }
      items.push({
  ...
  return { raw, prod, dev, transitive, items };
}
...
function parseSemgrep(report, { severityRemap = {}, excludePaths = [] } = {}) {
  ...
  for (const r of results) {
    ...
    if (pathMatchesAny(r.path, excludePaths)) {
      excludedByPath += 1;
      items.push({ id: r.check_id || null, path: r.path || null, line, severity: rawBand || null, disposition: 'excluded' });
      continue;
    }
    ...
    items.push({ id: r.check_id || null, path: r.path || null, line, severity: triagedBand || rawBand || null, disposition });
  }
  return { raw, triaged, remapped, excluded_by_path: excludedByPath, items };
}
...
function parseCredo(report) {
  ...
  for (const i of issues) {
    ...
    items.push({ scope: i.scope || null, file: i.filename || null, line: i.line_no != null ? i.line_no : null, cc: m ? Number(m[1]) : null });
  }
  return { tool: 'credo', violations: items.length, items };
}
...
function parseRubocop(report) {
  ...
  return { tool: 'rubocop', violations: items.length, items };
}
...
function parseLizard(report, { threshold = 10 } = {}) {
  ...
  for (const rawLine of text.split('\n')) {
    ...
    items.push({ scope: cols[7] || null, file: cols[6] || null, line: cols[9] ? Number(cols[9]) : null, cc });
  }
  return { tool: 'lizard', violations: items.length, items };
}
